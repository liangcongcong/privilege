...
<body>
	...
	<script>
	var grid;
	/*var pageForCombobox=1,pageSizeForCombobox=10;
	var totalPageForCombobox;
    var requestParam={
        	page:1,
        	rows:1
        };
	function setProjectQuantity(data) {
		totalPageForCombobox=Math.ceil(data.total/pageSizeForCombobox);
		//alert("totalPageForCombobox="+totalPageForCombobox);
    ...
    $(document).ready(function () {
        grid = $('#grid').edatagrid({
            ...
            saveUrl: '<%=basePath%>manage/role/add',
            updateUrl: '<%=basePath%>manage/role/update',
            //只传id
            destroyUrl: '<%=basePath%>manage/role/del',
				border : false,
				rownumbers : true,
				remoteSort : true,
				nowrap : false,
	...
	/*function previousComboboxDatas(){
		if(pageForCombobox==1){
			$.messager.show({
				title : "消息",
				msg : "当前已经是第一批数据了"
			});
		}else{
			pageForCombobox=pageForCombobox-1;
			$("#projectName").combobox({
				queryParams:{
					page:pageForCombobox,
					rows:pageSizeForCombobox
	...
	function nextComboboxDatas(){
		if(pageForCombobox==totalPageForCombobox){
			$.messager.show({
				title : "消息",
				msg : "当前已经是最后一排批数据了"
			});	
